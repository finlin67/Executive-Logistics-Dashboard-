...
// FILE: ExecutiveLogisticsDashboard.tsx
...
export default function ExecutiveLogisticsDashboard() {
  ...
  const [filterMode, setFilterMode] = useState<'all' | 'active' | 'warning'>('all');
  ...
  return (
    <div className="w-full h-full aspect-square flex items-center justify-center bg-slate-900 font-display antialiased text-white p-2 sm:p-4">
      <motion.div 
      ...
      >
        ...
        <div className="absolute top-0 right-0 w-48 h-48 bg-accent-cyan/10 blur-[80px] rounded-full -mr-24 -mt-24 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-48 h-48 bg-accent-indigo/10 blur-[80px] rounded-full -ml-24 -mb-24 pointer-events-none"></div>
        ...
        <div className="w-full h-full p-4 flex flex-col gap-4 relative z-10">
          ...
          <div className="flex items-center justify-between px-2 shrink-0">
            <div className="flex items-center gap-2">
              <Network className="text-accent-cyan w-5 h-5" />
              <h2 className="font-bold text-sm tracking-tight">
                Executive Dashboard <span className="text-xs font-normal text-white/40 ml-1">v2.4</span>
              </h2>
            </div>
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1 text-[10px] font-bold text-accent-cyan uppercase tracking-tighter">
          ...
          <div className="relative flex-1 w-full bg-slate-900/50 rounded-xl overflow-hidden border border-white/5 group z-0 min-h-0">
            ...
            <LogisticsMap hubs={hubs} filterMode={filterMode} />
            ...
            {/* Legend / Overlay Controls (Top Right) */}
            <div className="absolute right-3 top-3 flex flex-col gap-1.5 pointer-events-none">
              <TooltipIcon icon={<Layers className="w-3 h-3 text-accent-cyan" />} color="border-l-accent-cyan" label="TMS Active" />
              <TooltipIcon icon={<Warehouse className="w-3 h-3 text-accent-indigo" />} color="border-l-accent-indigo" label="WMS Active" />
              <TooltipIcon icon={<Building2 className="w-3 h-3 text-primary" />} color="border-l-primary" label="ERP Active" />
            </div>

            <div className="absolute bottom-4 left-4 flex flex-col gap-0.5 pointer-events-none">
              <div className="text-[10px] font-black text-accent-cyan/60 uppercase tracking-widest">Global Nodes</div>
          ...
          <div className="grid grid-cols-2 gap-3 h-auto sm:h-[240px] shrink-0">
            {/* Card 1: Supply Chain */}
            <div className="bg-slate-900/70 backdrop-blur-md border border-white/10 p-3 sm:p-4 rounded-xl flex flex-col justify-between border-l-4 border-l-accent-cyan hover:bg-slate-800/50 transition-colors">
              <div className="flex items-start justify-between">
                ...
                <Database className="text-accent-cyan w-4 h-4 sm:w-5 sm:h-5" />
              </div>
            ...
            {/* Card 2: Network Flow */}
            <div className="bg-slate-900/70 backdrop-blur-md border border-white/10 p-3 sm:p-4 rounded-xl flex flex-col justify-between border-l-4 border-l-accent-indigo hover:bg-slate-800/50 transition-colors">
              <div className="flex items-start justify-between">
                <div>
                ...
                <Truck className="text-accent-indigo w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <div className="flex items-end justify-between mt-2 sm:mt-0">
                <div className="flex flex-col">
                  <span className="text-xl sm:text-2xl font-black text-accent-cyan flex">
...
function LogisticsMap({ hubs, filterMode }: { hubs: Hub[], filterMode: 'all' | 'active' | 'warning' }) {
      ...
      <svg className="w-full h-full pointer-events-none">
        ...
        {ROUTES_DATA.map(([start, end], i) => {
          ...
          if (filterMode === 'active') {
            isVisible = h1.status === 'active' && h2.status === 'active';
          } else if (filterMode === 'warning') {
            isVisible = h1.status === 'warning' || h2.status === 'warning';
          }
        ...
        <g className="pointer-events-auto">
           {hubs.map(hub => {
             // Filter Logic for Hubs
             let isHubVisible = true;
             // Determine route connectivity for visibility
             const connectedToVisibleRoute = ROUTES_DATA.some(([start, end]) => {
                if (start !== hub.id && end !== hub.id) return false;
                const otherId = start === hub.id ? end : start;
                const otherHub = hubs.find(h => h.id === otherId);
                if (!otherHub) return false;
                
                if (filterMode === 'active') return hub.status === 'active' && otherHub.status === 'active';
                if (filterMode === 'warning') return hub.status === 'warning' || otherHub.status === 'warning';
                return true;
             });

             if (filterMode !== 'all') {
                // Show if part of a visible route OR strictly matches the filter status
                if (!connectedToVisibleRoute) {
                    if (filterMode === 'active' && hub.status !== 'active') isHubVisible = false;
                    if (filterMode === 'warning' && hub.status !== 'warning') isHubVisible = false;
                }
             }
...
function FilterButton({ active, onClick, label, color = "bg-accent-cyan" }: any) {
  return (
    <button 
      onClick={onClick}
      className={`
        px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all
        ${active ? `${color} text-white shadow-[0_0_15px_rgba(0,0,0,0.3)] scale-105` : 'bg-slate-800/80 text-white/40 hover:text-white hover:bg-slate-700'}
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-end mt-1">
        ...
        <span className="text-sm font-bold text-accent-cyan">{hub.volume}%</span>
      </div>
    </motion.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        <MapPin className="text-accent-indigo w-4 h-4" />
        <div>
